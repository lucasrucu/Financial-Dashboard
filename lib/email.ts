...
//   without verifying a domain. Set RESEND_FROM once you verify qori.land.
...
export async function sendAccessRequestEmail(
  ...
  const from = process.env.RESEND_FROM ?? "Qori <onboarding@resend.dev>";

  const lines = [
    `New access request for Qori.`,
    ``,
    `Email: ${payload.email}`,
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `Qori access request: ${payload.email}`,
        text: lines.join("\n"),
      }),
    });
